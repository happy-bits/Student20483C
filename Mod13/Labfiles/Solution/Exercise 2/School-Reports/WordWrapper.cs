...
namespace School
{
    class WordWrapper : IDisposable 
    {
        ...
        {
            // OO: Vid kryptering används PRIVATA nyckeln
            ...
            using (var algorithm = new AesManaged())
            {
                ...
                using (var inStream = new MemoryStream(bytesToDecrypt))
                {
                    ...
                    inStream.Seek(0, SeekOrigin.Begin);   // OO: sätt offeset till början av strömmen
                    inStream.Read(keyLength, 0, keyLength.Length); // OO: läs till till arrayen "keyLength" (från strömmen)
                    inStream.Read(ivLength, 0, ivLength.Length);

                    // OO: Omvandla byte-arrayen till heltal (!)
                    ...
                    using (var outStream = new MemoryStream())
                    {
                        ...
                        using (var decryptor = algorithm.CreateDecryptor(decryptedKey, iv))
                        {
                            ...
                            using (var decrypt = new CryptoStream(outStream, decryptor, CryptoStreamMode.Write)) // OO: "outStream" är "target"
                            {

                                // Exercise 2: Task 1n: Write all the data to the stream.
                                decrypt.Write(encryptedData, 0, dataLength);  // OO: skriver till "decrypt"-strömmen => leder till att "outStream" uppdateras
