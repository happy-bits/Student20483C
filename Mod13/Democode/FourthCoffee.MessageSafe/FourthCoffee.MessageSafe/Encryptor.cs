...
namespace FourthCoffee.MessageSafe
{
    ...
    class Encryptor : IDisposable
    {
        ...
        {
            // OO: Ett hash skapas med en kombination av ett (tidigare) valt "salt" och lösenordet
            var passwordHash = this.GeneratePasswordHash(password);

            // OO: Skapa en nykel och "IV" utifrån samma hash
            ...
            var transformer = this._algorithm.CreateEncryptor(key, iv);  //OO: Inbyggd
        ...
        // OO: Returnera bytes[] utfrån indata obh transformer
        ...
        private byte[] TransformBytes(ICryptoTransform transformer, byte[] bytesToTransform)
        {
            ...
            // Om lösenordet är fel skickas ett Cryptographic-exception här
