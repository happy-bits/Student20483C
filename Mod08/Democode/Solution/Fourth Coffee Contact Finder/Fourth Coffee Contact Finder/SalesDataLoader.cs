...
namespace Fourth_Coffee_Contact_Finder
{
    public class SalesDataLoader
    {
        ...
        public SalesPerson GetPersonByEmail(string email)
        {
            ...
            var rawData = Encoding.Default.GetBytes(           // OO: returnera en byte-array
        ...
        private void WriteDataToRequestStream(byte[] data)
        {
            ...
            dataStream.Write(data, 0, data.Length); // OO: 0 är offeset, var skrivandet ska börja
            dataStream.Close();
        }
        ...
        private SalesPerson ReadDataFromResponseStream()
        {
            ...
            var result = SalesPerson.FromJson(stream.BaseStream);           // OO: Vi förväntar oss att strömmen innehåller en SalePerson
            stream.Close();
