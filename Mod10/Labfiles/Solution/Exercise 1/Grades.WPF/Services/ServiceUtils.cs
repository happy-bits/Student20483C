...
namespace Grades.WPF.Services
{
    class ServiceUtils
    {
        ...
        #region Teacher
        public async Task<Teacher> GetTeacher(string userName)  //OO: 2a (async och Taskl)
        {
            if (!IsConnected())
                return null;

            var teacher = await Task.Run(() =>                  // OO: 2b (await Task.Run)
        ...
        public async Task GetStudentsByTeacher(string teacherName, Action<IEnumerable<Student>> callback)   // OO: 3e (async + callback)
        {
            if (!IsConnected())
                return;

            // Fetch students by using the GradesService service
            var students = await Task.Run(() =>                                     // OO: 3f (await och Task)
