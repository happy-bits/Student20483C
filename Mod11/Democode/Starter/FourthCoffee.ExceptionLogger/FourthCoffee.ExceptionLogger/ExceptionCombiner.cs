...
namespace FourthCoffee.ExceptionLogger
{
    ...
    public class ExceptionCombiner
    {
        ...
        public void WriteToWordDocument()
        {
            ...
            this._word = new Application();              // OO: under namespacet "Microsoft.Office.Interop.Word"
        ...
        private void AppendText(string text, bool bold, bool underLine)
        {
            ...
            currentLocation.InsertAfter(text);      // OO: inbyggd
        }
        ...
        private void InsertCarriageReturn()
        {
            ...
            currentLocation.InsertBreak(WdBreakType.wdLineBreak); // OO: en enum (finns även t.ex PageBreak
       
        }

        private void Save()
        {
            // OO: Ta bort filen om den finns
            if (File.Exists(this._outputFilePath))
                File.Delete(this._outputFilePath);


            var currentDocument = this._word.ActiveDocument; // OO: "currentDocument" är "dynamic"

            currentDocument.SaveAs(this._outputFilePath); // OO: kan inte F12'a
