...
namespace FourthCoffee.Employees
{
    class Program
    {
        ...
        private static void GroupingData()
        {
            ...
            foreach (var emp in emps)
            {
                Console.WriteLine(emp.Job);     // OO: "Job" är nyckeln till gruppen
                foreach (var name in emp.Names) // OO: "Names" innehåller alla namn i gruppen
                {
        ...
        private static void AggregatingData()
        {
            var jobs = from e in DBContext.Employees
                       group e by e.JobTitle into eGroup
                       select new { Job = eGroup.Key, CountOfEmployees = eGroup.Count() }; // OO: "Count" är aggregatet
